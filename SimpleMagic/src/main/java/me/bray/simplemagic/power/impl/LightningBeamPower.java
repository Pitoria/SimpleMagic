...
public class LightningBeamPower implements WandPower {

    private final SimpleMagic plugin;

    public LightningBeamPower(SimpleMagic plugin) {
        this.plugin = plugin;
    }
    ...
    public void cast(Player player, ItemStack wand) {
        ...
        double range = plugin.getPowerFile().getConfig()
                .getDouble("lightning-beam.range", 18);

        double damage = plugin.getPowerFile().getConfig()
                .getDouble("lightning-beam.damage", 1.5);

        int stunDuration = plugin.getPowerFile().getConfig()
                .getInt("lightning-beam.stun-duration", 20);

        int duration = plugin.getPowerFile().getConfig()
                .getInt("lightning-beam.duration", 60);
